...
function updateUIForDoll(index) {
  ...
  if (doll.id == -1) {
    ...
    if (showBuffedStats) {
      ...
      $('#doll' + (index + 1) + ' .rof span.base').text(doll.battle.finalstats.effective_rof);
      $('#doll' + (index + 1) + ' .rof span.waste').text(doll.battle.finalstats.rof_waste);
      $('#doll' + (index + 1) + ' .crit span.base').text(doll.battle.finalstats.crit + '%');
      $('#doll' + (index + 1) + ' .crit span.waste').text(doll.battle.finalstats.crit_waste);
...
function initDollsForBattle() {
  for (var i = 0; i < 5; i++) {
    ...
    doll.battle.maxstats = {
      ...
      effective_rof: doll.pre_battle.rof,
...
function calculateBattleStats(dollIndex) {
  ...
  doll.battle.minstats.rof_uncapped = Math.max(doll.battle.minstats.rof_uncapped, doll.battle.rof_uncapped);
  doll.battle.minstats.crit = Math.min(doll.battle.minstats.crit, doll.battle.crit);
  doll.battle.minstats.crit_uncapped = Math.max(doll.battle.minstats.crit_uncapped, doll.battle.crit_uncapped);
